...
class MDBlockParser {
  MDBlockParser(this.lines, this.document) {
    blockSyntaxes
  ...
  final List<MDBlockSyntax> standardBlockSyntaxes = [
    const MDEmptyBlockSyntax(),
    const MDBlockTagBlockHtmlSyntax(),
    MDLongBlockHtmlSyntax(r'^ {0,3}<pre(?:\s|>|$)', '</pre>'),
    MDLongBlockHtmlSyntax(r'^ {0,3}<script(?:\s|>|$)', '</script>'),
    MDLongBlockHtmlSyntax(r'^ {0,3}<style(?:\s|>|$)', '</style>'),
    MDLongBlockHtmlSyntax('^ {0,3}<!--', '-->'),
    MDLongBlockHtmlSyntax('^ {0,3}<\\?', '\\?>'),
    MDLongBlockHtmlSyntax('^ {0,3}<![A-Z]', '>'),
    MDLongBlockHtmlSyntax('^ {0,3}<!\\[CDATA\\[', '\\]\\]>'),
    const OtherTagBlockHtmlSyntax(),
    const SetextHeaderSyntax(),
    const HeaderSyntax(),
    const MDCodeBlockSyntax(),
    const MDBlockquoteSyntax(),
    const MDHorizontalRuleSyntax(),
    const MDUnorderedListSyntax(),
    const OrderedListSyntax(),
    const MDParagraphSyntax()
  ];
...
abstract class MDBlockSyntax {
  ...
  bool canParse(MDBlockParser parser) {
    return pattern!.firstMatch(parser.current) != null;
  }

  MDNode? parse(MDBlockParser parser);

  List<String?> parseChildLines(MDBlockParser parser) {
  ...
  static bool isAtBlockEnd(MDBlockParser parser) {
    if (parser.isDone) {
      return true;
    }
  ...
  static String generateAnchorHash(MDElement element) =>
...
class MDEmptyBlockSyntax extends MDBlockSyntax {
  const MDEmptyBlockSyntax();
...
class SetextHeaderSyntax extends MDBlockSyntax {
  const SetextHeaderSyntax();
...
class SetextHeaderWithIdSyntax extends SetextHeaderSyntax {
  ...
  @override
  MDNode parse(MDBlockParser parser) {
    final element = super.parse(parser) as MDElement;
    element.generatedId = MDBlockSyntax.generateAnchorHash(element);
    return element;
  }
}

/// Parses atx-style headers: `## Header ##`.
class HeaderSyntax extends MDBlockSyntax {
  const HeaderSyntax();
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    return MDElement('h$level', [contents]);
  }
}

/// Parses atx-style headers, and adds generated IDs to the generated elements.
class MDHeaderWithIdSyntax extends HeaderSyntax {
...
class MDBlockquoteSyntax extends MDBlockSyntax {
  const MDBlockquoteSyntax();
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    final children = MDBlockParser(childLines, parser.document).parseLines();
    return MDElement('blockquote', children);
  }
}

/// Parses preformatted code blocks that are indented four spaces.
class MDCodeBlockSyntax extends MDBlockSyntax {
  const MDCodeBlockSyntax();
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    return MDElement('pre', [MDElement.text('code', escaped)]);
  }
}
...
class MDFencedCodeBlockSyntax extends MDBlockSyntax {
  ...
  @override
  List<String> parseChildLines(MDBlockParser parser, [String? endBlock]) {
    endBlock ??= '';
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    final code = MDElement.text('code', childLines.join('\n'));
    ...
    final element = MDElement('pre', [code]);
    return element;
  }
}

/// Parses horizontal rules like `---`, `_ _ _`, `*  *  *`, etc.
class MDHorizontalRuleSyntax extends MDBlockSyntax {
  const MDHorizontalRuleSyntax();
...
abstract class MDBlockHtmlSyntax extends MDBlockSyntax {
  const MDBlockHtmlSyntax();

  @override
  bool get canEndBlock => true;
}

class MDBlockTagBlockHtmlSyntax extends MDBlockHtmlSyntax {
  const MDBlockTagBlockHtmlSyntax();
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    return MDText(childLines.join('\n'));
  }
}

class OtherTagBlockHtmlSyntax extends MDBlockTagBlockHtmlSyntax {
  const OtherTagBlockHtmlSyntax();
...
class MDLongBlockHtmlSyntax extends MDBlockHtmlSyntax {
  MDLongBlockHtmlSyntax(String pattern, String endPattern)
...
abstract class MDListSyntax extends MDBlockSyntax {
  const MDListSyntax();
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    while (!parser.isDone) {
      ...
      } else if (MDBlockSyntax.isAtBlockEnd(parser)) {
        // Done with the list.
        break;
      } else {
    ...
    for (final item in items) {
      final itemParser = MDBlockParser(item.lines, parser.document);
      final children = itemParser.parseLines();
      itemNodes.add(MDElement('li', children));
    ...
    if (listIsTight) {
      ...
      for (final item in itemNodes) {
        for (var i = 0; i < item.children!.length; i++) {
          ...
          if (child is MDElement && child.tag == 'p') {
    ...
    if (listTag == 'ol' && startNumber != 1) {
      return MDElement(listTag, itemNodes)..attributes['start'] = '$startNumber';
    } else {
      return MDElement(listTag, itemNodes);
    }
  }
...
class MDUnorderedListSyntax extends MDListSyntax {
  const MDUnorderedListSyntax();
...
class OrderedListSyntax extends MDListSyntax {
  const OrderedListSyntax();
...
/// Parses tables.
class MDTableSyntax extends MDBlockSyntax {
  const MDTableSyntax();
  ...
  MDNode? parse(MDBlockParser parser) {
    ...
    final head = MDElement('thead', [headRow]);

    // Advance past the divider of hyphens.
    parser.advance();

    final rows = <MDElement>[];
    while (!parser.isDone && !MDBlockSyntax.isAtBlockEnd(parser)) {
      final row = parseRow(parser, alignments, 'td');
      while (row.children!.length < columnCount) {
        // Insert synthetic empty cells.
        row.children!.add(MDElement.empty('td'));
      }
      while (row.children!.length > columnCount) {
    ...
    if (rows.isEmpty) {
      return MDElement('table', [head]);
    } else {
      final body = MDElement('tbody', rows);

      return MDElement('table', [head, body]);
    }
  }
  ...
  MDElement parseRow(
      MDBlockParser parser, List<String?> alignments, String cellType) {
    final line = parser.current
    ...
    for (var cell in cells) {
      ...
      row.add(MDElement(cellType, [contents]));
    }
    ...
    return MDElement('tr', row);
  }
}

/// Parses paragraphs of regular text.
class MDParagraphSyntax extends MDBlockSyntax {
  const MDParagraphSyntax();
  ...
  @override
  bool canParse(MDBlockParser parser) => true;
  ...
  MDNode parse(MDBlockParser parser) {
    ...
    while (!MDBlockSyntax.isAtBlockEnd(parser)) {
      childLines.add(parser.current);
      parser.advance();
    }
  ...
  List<String>? _extractReflinkDefinitions(
      MDBlockParser parser, List<String> lines) {
  ...
  bool _parseReflinkDefinition(MDBlockParser parser, String contents) {
    ...
    parser.document.linkReferences
        .putIfAbsent(label, () => MDLinkReference(label, destination!, title!));
    return true;
  }
}
