...
class MDInlineParser {
  MDInlineParser(this.source, this.document) : _stack = <MDTagState>[] {
    ...
    if (documentHasCustomInlineSyntaxes) {
      ...
      syntaxes.add(MDTextSyntax(r'[A-Za-z0-9]+(?=\s)'));
    } else {
      syntaxes.add(MDTextSyntax(r'[ \tA-Za-z0-9]*[A-Za-z0-9](?=\s)'));
    }
    ...
    // Custom link resolvers go after the generic text syntax.
      ..insertAll(1, [
        MDLinkSyntax(linkResolver: document.linkResolver),
        MDImageSyntax(linkResolver: document.imageLinkResolver)
      ]);
  }

  static final List<MDInlineSyntax> _defaultSyntaxes =
  List<MDInlineSyntax>.unmodifiable(<MDInlineSyntax>[
    MDEmailAutolinkSyntax(),
    MDAutolinkSyntax(),
    MDLineBreakSyntax(),
    MDLinkSyntax(),
    MDImageSyntax(),
    ...
    MDTagSyntax(r'\*+', requiresDelimiterRun: true),
    // Parse "__strong__" and "_emphasis_" tags.
    MDTagSyntax(r'_+', requiresDelimiterRun: true),
    MDCodeSyntax(),
  ...
  final List<MDInlineSyntax> syntaxes = <MDInlineSyntax>[];
  ...
  final List<MDTagState> _stack;

  List<MDNode>? parse() {
    // Make a fake top tag to hold the results.
    _stack.add(MDTagState(0, 0, null, null));
  ...
  void writeTextRange(int start, int end) {
    ...
    if (nodes.isNotEmpty && nodes.last is MDText) {
      final textNode = nodes.last as MDText;
      nodes[nodes.length - 1] = MDText('${textNode.text}$text');
    } else {
      nodes.add(MDText(text));
    }
  }

  /// Add [node] to the last [MDTagState] on the stack.
  void addNode(MDNode node) {
    _stack.last.children.add(node);
  }

  /// Push [state] onto the stack of [MDTagState]s.
  void openTag(MDTagState state) => _stack.add(state);
...
abstract class MDInlineSyntax {
  MDInlineSyntax(String pattern) : pattern = RegExp(pattern, multiLine: true);
  ...
  bool tryMatch(MDInlineParser parser, [int? startMatchPos]) {
  ...
  bool onMatch(MDInlineParser parser, Match match);
}

/// Represents a hard line break.
class MDLineBreakSyntax extends MDInlineSyntax {
  MDLineBreakSyntax() : super(r'(?:\\|  +)\n');

  /// Create a void <br> element.
  @override
  bool onMatch(MDInlineParser parser, Match match) {
    parser.addNode(MDElement.empty('br'));
    return true;
  }
}

/// Matches stuff that should just be passed through as straight text.
class MDTextSyntax extends MDInlineSyntax {
  MDTextSyntax(String pattern, {String? sub})
      : substitute = sub,
        super(pattern);
  ...
  bool onMatch(MDInlineParser parser, Match match) {
    ...
    parser.addNode(MDText(substitute!));
    return true;
  }
}

/// Escape punctuation preceded by a backslash.
class MDEscapeSyntax extends MDInlineSyntax {
  MDEscapeSyntax() : super(r'''\\[!"#$%&'()*+,\-./:;<=>?@\[\\\]^_`{|}~]''');

  @override
  bool onMatch(MDInlineParser parser, Match match) {
    // Insert the substitution.
    parser.addNode(MDText(match[0]![1]));
    return true;
  }
}
...
class MDInlineHtmlSyntax extends MDTextSyntax {
  MDInlineHtmlSyntax() : super(r'<[/!?]?[A-Za-z][A-Za-z0-9-]*(?:\s[^>]*)?>');
}

/// Matches autolinks like `<foo@bar.example.com>`.
///
/// See <http://spec.commonmark.org/0.28/#email-address>.
class MDEmailAutolinkSyntax extends MDInlineSyntax {
  MDEmailAutolinkSyntax() : super('<($_email)>');
  ...
  bool onMatch(MDInlineParser parser, Match match) {
    final url = match[1]!;
    final anchor = MDElement.text('a', escapeHtml(url));
...
class MDAutolinkSyntax extends MDInlineSyntax {
  MDAutolinkSyntax() : super(r'<(([a-zA-Z][a-zA-Z\-\+\.]+):(?://)?[^\s>]*)>');
...
/// Matches autolinks like `http://foo.com`.
class MDAutolinkExtensionSyntax extends MDInlineSyntax {
  MDAutolinkExtensionSyntax() : super('$start(($scheme)($domain)($path))');
...
class _DelimiterRun {
  ...
  static _DelimiterRun? tryParse(
      MDInlineParser parser, int runStart, int runEnd) {
    bool leftFlanking,
        rightFlanking,
...
class MDTagSyntax extends MDInlineSyntax {
  MDTagSyntax(String pattern, {String? end, this.requiresDelimiterRun = false})
  ...
  bool onMatch(MDInlineParser parser, Match match) {
    ...
    if (!requiresDelimiterRun) {
      parser.openTag(MDTagState(parser.pos, matchEnd + 1, this, null));
      return true;
    }

    final delimiterRun = _DelimiterRun.tryParse(parser, matchStart, matchEnd);
    if (delimiterRun != null && delimiterRun.canOpen) {
      parser.openTag(MDTagState(parser.pos, matchEnd + 1, this, delimiterRun));
      return true;
    } else {
  ...
  bool onMatchEnd(MDInlineParser parser, Match match, MDTagState state) {
    final runLength = match.group(0)!.length;
    final matchStart = parser.pos;
    final matchEnd = parser.pos + runLength - 1;
    ...
    if (openingRunLength == 1 && runLength == 1) {
      parser.addNode(MDElement('em', state.children));
    } else if (openingRunLength == 1 && runLength > 1) {
      parser
        ..addNode(MDElement('em', state.children))
        ..pos = parser.pos - (runLength - 1)
        ..start = parser.pos;
    } else if (openingRunLength > 1 && runLength == 1) {
      parser
        ..openTag(
            MDTagState(state.startPos, state.endPos - 1, this, delimiterRun))
        ..addNode(MDElement('em', state.children));
    } else if (openingRunLength == 2 && runLength == 2) {
      parser.addNode(MDElement('strong', state.children));
    } else if (openingRunLength == 2 && runLength > 2) {
      parser
        ..addNode(MDElement('strong', state.children))
        ..pos = parser.pos - (runLength - 2)
        ..start = parser.pos;
    } else if (openingRunLength > 2 && runLength == 2) {
      parser
        ..openTag(
            MDTagState(state.startPos, state.endPos - 2, this, delimiterRun))
        ..addNode(MDElement('strong', state.children));
    } else if (openingRunLength > 2 && runLength > 2) {
      parser
        ..openTag(
...
class MDStrikethroughSyntax extends MDTagSyntax {
  MDStrikethroughSyntax() : super('~+', requiresDelimiterRun: true);
  ...
  bool onMatchEnd(MDInlineParser parser, Match match, MDTagState state) {
    ...
    parser.addNode(MDElement('del', state.children));
    return true;
  }
}

/// Matches links like `[blah][label]` and `[blah](url)`.
class MDLinkSyntax extends MDTagSyntax {
  MDLinkSyntax({Resolver? linkResolver, String pattern = r'\['})
  ...
  /// resolve [label] and [state] into a [MDNode]. If [label] is defined in
  /// [linkReferences] or can be resolved by [linkResolver], returns a [MDNode]
  /// that links to the resolved URL.
  ///
  /// Otherwise, returns `null`.
  ///
  /// [label] does not need to be normalized.
  MDNode? _resolveReferenceLink(
      String label, MDTagState state, Map<String, MDLinkReference> linkReferences) {
  ...
  MDNode _createNode(MDTagState state, String destination, String? title) {
    final element = MDElement('a', state.children);
  ...
  bool _tryAddReferenceLink(MDInlineParser parser, MDTagState state, String label) {
    final element =
  ...
  //
  // Returns whether the link was added successfully.
  bool _tryAddInlineLink(MDInlineParser parser, MDTagState state, MDInlineLink link) {
  ...
  String? _parseReferenceLinkLabel(MDInlineParser parser) {
  ...
  /// Parse an inline [MDInlineLink] at the current position.
  ///
  ...
  /// `(http://url "title")`.
  ///
  /// Returns the [MDInlineLink] if one was parsed, or `null` if not.
  MDInlineLink? _parseInlineLink(MDInlineParser parser) {
  ...
  MDInlineLink? _parseInlineBracketedLink(MDInlineParser parser) {
    parser.advanceBy(1);
    ...
    if (char == $space || char == $lf || char == $cr || char == $ff) {
      ...
      return MDInlineLink(destination, title: title);
    } else if (char == $rparen) {
      return MDInlineLink(destination);
    } else {
  ...
  /// character of the destination.
  MDInlineLink? _parseInlineBareDestinationLink(MDInlineParser parser) {
    // According to
  ...
  void _moveThroughWhitespace(MDInlineParser parser) {
    while (true) {
      final char = parser.charAt(parser.pos);
      if (char != $space &&
          char != $tab &&
  ...
  // destination.
  String? _parseTitle(MDInlineParser parser) {
    _moveThroughWhitespace(parser);
    if (parser.isDone) {
      return null;
    }
...
class MDImageSyntax extends MDLinkSyntax {
  MDImageSyntax({Resolver? linkResolver})
      : super(linkResolver: linkResolver, pattern: r'!\[');

  @override
  MDNode _createNode(MDTagState state, String destination, String? title) {
    final element = MDElement.empty('img');
...
class MDCodeSyntax extends MDInlineSyntax {
  MDCodeSyntax() : super(_pattern);
  ...
  @override
  bool onMatch(MDInlineParser parser, Match match) {
    parser.addNode(MDElement.text('code', escapeHtml(match[2]!.trim())));
    return true;
  }
}
...
class MDTagState {
  MDTagState(this.startPos, this.endPos, this.syntax, this.openingDelimiterRun)
      : children = <MDNode>[];
  ...
  /// pattern.
  bool tryMatch(MDInlineParser parser) {
    final endMatch =
  ...
  List<MDNode>? close(MDInlineParser parser, Match? endMatch) {
...
class MDInlineLink {
  MDInlineLink(this.destination, {this.title});
