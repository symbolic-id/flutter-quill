...
class DeltaMarkdownDecoder extends Converter<String, String> {
  ...
  String convert(String input) {
    ...
    final markdownDocument = MDDocument().parseLines(lines);

    return jsonEncode(_DeltaVisitor().convert(markdownDocument).toJson());
  }
}

class _DeltaVisitor implements ast.MDNodeVisitor {
  ...
  ast.MDElement? previousElement;
  late ast.MDElement previousToplevelElement;

  Delta convert(List<ast.MDNode> nodes) {
    delta = Delta();
  ...
  @override
  void visitText(ast.MDText text) {
  ...
  @override
  bool visitElementBefore(ast.MDElement element) {
  ...
  @override
  void visitElementAfter(ast.MDElement element) {
    if (element.tag == 'li' &&
        (previousToplevelElement.tag == 'ol' ||
  ...
  Attribute? _tagToAttribute(ast.MDElement el) {
    switch (el.tag) {
      case 'em':
...
class MDImageAttribute extends Attribute<String?> {
  MDImageAttribute(String? val) : super('image', AttributeScope.EMBEDS, val);
}

class MDDividerAttribute extends Attribute<String?> {
  MDDividerAttribute() : super('divider', AttributeScope.EMBEDS, 'hr');
}
